...
def load_leads():
    ...
    for f in sorted(glob.glob(os.path.join(LEAD_DIR, "m2_lead_L*.json"))):
        ...
        # NMI version: recompute on-the-fly using the 6-anchor calibration
        # (the rho_cal / kj_dft_cal fields in the JSON are STALE, computed under
        # the older 2-anchor calibration that produced inflated headline numbers).
        # 6-anchor coefficients per main paper Methods, "Six-anchor DFT calibration":
        #     rho_cal = 1.392 * rho_DFT - 0.415
        #     HOF_cal = HOF_DFT - 206.7 kJ/mol
        # K-J D and P are taken from the explicit per-lead overrides below
        # (sourced from main paper Table 1 / Supplementary Table S1).
        rho_dft = _safe_float(s.get("rho_dft"))
        rho_cal = (1.392 * rho_dft - 0.415) if rho_dft is not None else None
        HOF_dft = _safe_float(s.get("HOF_kJmol_wb97xd"))
        HOF_cal = (HOF_dft - 206.7) if HOF_dft is not None else None
        # 6-anchor calibrated D, P from main paper Table 1 / SI Table S1
        SIX_ANCHOR_DP = {
            "L1":  (8.25, 32.9), "L2":  (7.78, 28.1), "L3":  (7.50, 26.5),
            "L4":  (6.71, 20.9), "L5":  (7.99, 29.6), "L9":  (7.33, 24.6),
            "L11": (7.88, 28.5), "L13": (7.36, 24.5), "L16": (7.50, 26.5),
            "L18": (7.09, 22.6), "L19": (7.24, 24.0), "L20": (7.41, 25.8),
        }
        D_cal, P_cal = SIX_ANCHOR_DP.get(d["id"], (None, None))
...
def build_figure(leads):
    ...
    # Suptitle removed; figcaption in NMIPaper.html (Extended Data Fig. 6)
    # carries the headline. Vestigial "Figure 19" was the long-form-paper
    # numbering and does not apply to the NMI version.
    return fig

"""ED Fig. 9 - Distribution-learning small multiples (MOSES-style metrics).

Brought over verbatim from `EnergeticDiffusion2/plot_moses_panels.py`.
Only path constants (ROOT, OUT_DIR) updated for this repo.

Six panels: validity, uniqueness, novelty, internal-diversity, FCD,
scaffold-NN-Tanimoto. Compares SMILES-LSTM (red) vs seven DGLD conditions
(blue: C0, C1v, C1vs, C2vs, C2vsh, C3h, C3sa).

Inputs (small JSON files in this repo):
  data/results/fcd_results.json
  data/results/m6_post.json
  data/results/pool_metrics.json
"""
import json
import os
import numpy as np
...
ROOT = r"E:\Projects\DGLD4Energetic-public"
DATA_DIR = os.path.join(ROOT, "data", "results")
...
def main():
    fcd  = json.load(open(os.path.join(DATA_DIR, "fcd_results.json")))
    m6   = json.load(open(os.path.join(DATA_DIR, "m6_post.json")))
    pool = json.load(open(os.path.join(DATA_DIR, "pool_metrics.json")))
    per_run = m6["per_run"]

    intdiv: dict[str, list[float]] = {}
    for r in per_run:
        c = r["condition"]
